...
export class CatalogsService {
  constructor(
    @InjectRepository(PH_DOC_Catalogs)
    private readonly catalogs_rep: Repository<PH_DOC_Catalogs>,
  ) {}

  async create(res: Response, dto: CreateCatalogDto) {
    const object = await this.catalogs_rep.save(dto);
  ...
  async findAll(res: Response, query: QueryPaginationDto) {
    ...
    const [array_objects, total_items] = await this.catalogs_rep.findAndCount({
      skip: skip_items,
      take: take_items,
    });
  ...
  async findOne(res: Response, id: string) {
    const object = await this.catalogs_rep.findOneOrFail({
      where: { ph_ref: id },
    });
    const statusCode = 200;
  ...
  async update(res: Response, id: string, dto: UpdateCatalogDto) {
    const object = await this.catalogs_rep.findOneOrFail({
      where: { ph_ref: id },
    });
    const info = await this.catalogs_rep.update(id, dto);
    const statusCode = 200;
  ...
  async remove(res: Response, id: string) {
    ...
    const info = await this.catalogs_rep.delete({ ph_ref: id });
    const statusCode = 200;
