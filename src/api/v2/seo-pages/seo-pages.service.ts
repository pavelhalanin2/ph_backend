...
export class SEOPagesService {
  constructor(
    @InjectRepository(PH_CTL_SEOPages)
    private readonly seo_pages_rep: Repository<PH_CTL_SEOPages>,
  ) {}

  async create(res: Response, dto: CreateSEOPageDto) {
    const object = await this.seo_pages_rep.save(dto);
  ...
  async findAll(res: Response, query: QueryPaginationDto) {
    ...
    const [array_objects, total_items] = await this.seo_pages_rep.findAndCount({
      skip: skip_items,
      take: take_items,
    });
  ...
  async findOne(res: Response, id: string) {
    const object = await this.seo_pages_rep.findOneOrFail({
      where: { ph_ref: id },
    });
    const statusCode = 200;
  ...
  async update(res: Response, id: string, dto: UpdateSEOPageDto) {
    const object = await this.seo_pages_rep.findOneOrFail({
      where: { ph_ref: id },
    });
    const info = await this.seo_pages_rep.update(id, dto);
    const statusCode = 200;
  ...
  async remove(res: Response, id: string) {
    ...
    const info = await this.seo_pages_rep.delete({ ph_ref: id });
    const statusCode = 200;
