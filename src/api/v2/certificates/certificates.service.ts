...
export class CertificatesService {
  constructor(
    @InjectRepository(PH_DOC_Certificates)
    private readonly certificates_rep: Repository<PH_DOC_Certificates>,
  ) {}

  async create(res: Response, dto: CreateCertificateDto) {
    const object = await this.certificates_rep.save(dto);
  ...
  async findAll(res: Response, query: QueryPaginationDto) {
    ...
    const [array_objects, total_items] = await this.certificates_rep.findAndCount({
      skip: skip_items,
      take: take_items,
    });
  ...
  async findOne(res: Response, id: string) {
    const object = await this.certificates_rep.findOneOrFail({
      where: { ph_ref: id },
    });
    const statusCode = 200;
  ...
  async update(res: Response, id: string, dto: UpdateCertificateDto) {
    const object = await this.certificates_rep.findOneOrFail({
      where: { ph_ref: id },
    });
    const info = await this.certificates_rep.update(id, dto);
    const statusCode = 200;
  ...
  async remove(res: Response, id: string) {
    ...
    const info = await this.certificates_rep.delete({ ph_ref: id });
    const statusCode = 200;
