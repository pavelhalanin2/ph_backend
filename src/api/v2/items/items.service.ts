...
export class ItemsService {
  constructor(
    @InjectRepository(PH_CTL_Items)
    private readonly items_rep: Repository<PH_CTL_Items>,
  ) {}

  async create(res: Response, dto: CreateItemDto) {
    const object = await this.items_rep.save(dto);
  ...
  async findAll(res: Response, query: QueryPaginationDto) {
    ...
    const [array_objects, total_items] = await this.items_rep.findAndCount({
      skip: skip_items,
      take: take_items,
    });
  ...
  async findOne(res: Response, id: string) {
    const object = await this.items_rep.findOneOrFail({
      where: { ph_ref: id },
    });
    const statusCode = 200;
  ...
  async update(res: Response, id: string, dto: UpdateItemDto) {
    const object = await this.items_rep.findOneOrFail({
      where: { ph_ref: id },
    });
    const info = await this.items_rep.update(id, dto);
    const statusCode = 200;
  ...
  async remove(res: Response, id: string) {
    ...
    const info = await this.items_rep.delete({ ph_ref: id });
    const statusCode = 200;
