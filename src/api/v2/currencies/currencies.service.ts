...
export class CurrenciesService {
  constructor(
    @InjectRepository(PH_CTL_Currencies)
    private readonly currencies_rep: Repository<PH_CTL_Currencies>,
  ) {}

  async create(res: Response, dto: CreateCurrencyDto) {
    const object = await this.currencies_rep.save(dto);
  ...
  async findAll(res: Response, query: QueryPaginationDto) {
    ...
    const [array_objects, total_items] = await this.currencies_rep.findAndCount(
      {
        skip: skip_items,
        take: take_items,
      },
  ...
  async findOne(res: Response, id: string) {
    const object = await this.currencies_rep.findOneOrFail({
      where: { ph_ref: id },
    });
    const statusCode = 200;
  ...
  async update(res: Response, id: string, dto: UpdateCurrencyDto) {
    const object = await this.currencies_rep.findOneOrFail({
      where: { ph_ref: id },
    });
    const info = await this.currencies_rep.update(id, dto);
    const statusCode = 200;
  ...
  async remove(res: Response, id: string) {
    ...
    const info = await this.currencies_rep.delete({ ph_ref: id });
    const statusCode = 200;
