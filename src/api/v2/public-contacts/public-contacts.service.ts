...
export class PublicContactsService {
  constructor(
    @InjectRepository(PH_CTL_PublicContacts)
    private readonly public_contacts_rep: Repository<PH_CTL_PublicContacts>,
  ) {}

  async create(res: Response, dto: CreatePublicContactDto) {
    const object = await this.public_contacts_rep.save(dto);
  ...
  async findAll(res: Response, query: QueryPaginationDto) {
    ...
    const [array_objects, total_items] =
      await this.public_contacts_rep.findAndCount({
        skip: skip_items,
        take: take_items,
      });
  ...
  async findOne(res: Response, id: string) {
    const object = await this.public_contacts_rep.findOneOrFail({
      where: { ph_ref: id },
    });
    const statusCode = 200;
  ...
  async update(res: Response, id: string, dto: UpdatePublicContactDto) {
    const object = await this.public_contacts_rep.findOneOrFail({
      where: { ph_ref: id },
    });
    const info = await this.public_contacts_rep.update(id, dto);
    const statusCode = 200;
  ...
  async remove(res: Response, id: string) {
    ...
    const info = await this.public_contacts_rep.delete({ ph_ref: id });
    const statusCode = 200;
