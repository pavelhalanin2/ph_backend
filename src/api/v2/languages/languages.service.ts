...
export class LanguagesService {
  constructor(
    @InjectRepository(PH_CTL_Languages)
    private readonly languages_rep: Repository<PH_CTL_Languages>,
  ) {}

  async create(res: Response, dto: CreateLanguageDto) {
    const object = await this.languages_rep.save(dto);
  ...
  async findAll(res: Response, query: QueryPaginationDto) {
    ...
    const [array_objects, total_items] = await this.languages_rep.findAndCount({
      skip: skip_items,
      take: take_items,
    });
  ...
  async findOne(res: Response, id: string) {
    const object = await this.languages_rep.findOneOrFail({
      where: { ph_ref: id },
    });
    const statusCode = 200;
  ...
  async update(res: Response, id: string, dto: UpdateLanguageDto) {
    const object = await this.languages_rep.findOneOrFail({
      where: { ph_ref: id },
    });
    const info = await this.languages_rep.update(id, dto);
    const statusCode = 200;
  ...
  async remove(res: Response, id: string) {
    ...
    const info = await this.languages_rep.delete({ ph_ref: id });
    const statusCode = 200;
